...
export const createCustomer = async (item: Omit<Customer, "ref">): Promise<Customer> => {
...
export const loginCustomer = async (login: string, password: string): Promise<Customer> => {
...
export const loadCustomerPhoto = async (filename: string): Promise<string> => {
  const res = await apiClient.get<ArrayBuffer>(`/customers/loadPhoto/${filename}`, {
